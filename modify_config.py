...
try:
    ...
    try:
        ...
        for site in ordered_obj.get("sites", []):
            ...
            # 🛠️ 核心大招：网盘/磁力组件全线强行洗白去后缀，完美对齐未配Token自动消隐机制
            ...
            # 🛠️ 精准硬核锁定：唯独提取 key: 热播影视 实施专属首位置顶与长鸣谢注入
            ...
            if is_target_rebo_main:
                ...
                # 影视/追剧/APP大类（包括洗净的瓜子影视、以及保留原地队形的 key: rb 线路）
                if not raw_name.startswith("🦋"): raw_name = f"🦋 {raw_name}"
                site["name"] = raw_name
                site["category"] = "综合"
                block_2_yingshi.append(site)
        ...
        ordered_obj["sites"] = (
            ...
            block_9_fuli           # 9. 福利18禁安全空队坠尾 (绿色精简线此块全空)
        )
